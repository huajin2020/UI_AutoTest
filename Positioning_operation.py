...
class Mylib():
    ...
    #python自带的__init__方法，在（非）main主方法下无须手动调用都可以执行
    def __init__(self):
    ...
    def locate_element(self,locate_type,value):
        #涉及元素定位封装必须用return返回给函数，不然调用会报错；如果封装里还调用了send_keys()、click()等其它方法可以不
        # 返回
        return self.driver.find_element(locate_type,value)

    #如果在（非）main主方法下不想手动调用自定义的关闭浏览器的函数（方法），可以使用python自带的__def__方法（自动调用），
    # 不然执行不了close()或quit()
    def __del__(self):
    #def close(self):
        time.sleep(5)
